...
(define (make-scheme-number n)
  ((get 'make 'scheme-numner) n))

(define (instsall-rational-package)
  ;;internal procedures
  (define (numer x) (car x))
  (define (denom x) (cdr x))
  (define (make-rat n d)
...
(define (install-complex-package)
  ...
  (define (make-from-real-imag x y)
    ((get 'make-from-real-imag 'reacangular) x y))
  (define (make-from-mag-ang r a)
...
;;exercise 2.77
;;twice, one for obtaining the complex package and
;;another for obtaining the rectangular package
...
(define (type-tag exp)
  (cond ((pair? exp) (car exp))
	((number? exp) 'scheme-number)
	(else (error "Bad tagged exp -- TYPE-TAG" exp))))

(define (contents exp)
  (cond ((pair? exp) (cdr exp))
	((number? exp) exp)
	(else (error "Bad tagger exp -- CONTENTS" exp))))

;;exercise 2.79
;;(define (equ? x y) (apply-generic 'equ? x y))
;;(put 'equ? '(scheme-number scheme-number) =)

;;(define (equ-rat? x y) (and (equ? (number x) (number y))
;;			   (equ? (denom x) (denom y))))
